...
?>
  ...
  <header>
    ...
    <div id="topRecord">Record: <?php echo getRecord($username,$_GET['l']); ?></div>
  </header>
  <div class="main">
  <div class="grid">
    <span id="pos"></span>
    ...
    <div class="leads"><?php echo getLeaderboard($_GET['l']); ?></div>
  </div>
  </div>
  ...
  <script>
    $("#gameover").hide();$(".leads").toggle();
    var exp=<?php echo getUserExp($username); ?>,level=<?php echo getUserLevel($username); ?>,points=0,varz=3.69,score=0,gameover=false;
    var expCap=Math.floor(100*(Math.pow(1.1,level))),expToAdd=0,record=<?php echo getRecord($username,$_GET['l']); ?>,w=10,h=7,mode=0,tiles="",max=9;
    ...
    var numlvl=<?php echo $_GET['l']; ?>;
    procGeneration(<?php echo $_GET['l']; ?>);
    ...
    "pts</p><p>"+zerosRows()+" zeros rows: +"+zerosRowsPoints()+"pts</p><p>"+zerosCols()+" zeros columns: +"+zerosColsPoints()+'pts</p><br><br><div class="next">press N or ENTER to go to the NEXT level</div>');expToAdd=bonusExp();upload();applyExp()}
    function applyExp(){$("#progressbar").val(exp);$("#progressbar").prop("max",expCap);$("#output_exp").text("Exp: "+exp+"/"+expCap);$("#output_level").text("Level: "+level);0<expToAdd&&(expToAdd--,exp++,exp>=expCap&&(exp=0,level++),expCap=Math.floor(100*Math.pow(1.1,level)),setTimeout("applyExp()",20))}function upload(){$.get("upload.php?level="+numlvl+"&score="+Math.floor(score/varz))}
    function move(a){pos.moving||(36<a&&41>a?pos.move(a-37):65==a?pos.move(0):87==a?pos.move(1):68==a?pos.move(2):83==a&&pos.move(3));82==a&&location.reload();if(88==a||27==a)window.location="./levels.php";76==a&&$(".leads").toggle("slow");!gameover||13!=a&&78!=a||(window.location="./grid.php?l="+(numlvl+1))}$(document).keydown(function(a){move(a.which);a.preventDefault()});
  </script>
